...
export class MarketingService {
    ...
    try {
      ...
      this.unsub = onSnapshot(
        ...
        err => {
          ...
          this.error.set(this.mapFirestoreError(err));
          this.loading.set(false);
        },
      );
    } catch (err) {
  ...
  async subscribe(input: MarketingSubscribeInput): Promise<SubscribeResult> {
    ...
    try {
      ...
      console.error('[Marketing]', err);
      throw new Error(this.mapFirestoreError(err));
    }
  }
  ...
  private mapFirestoreError(err: unknown): string {
    if (isPlatformBrowser(this.pid) && typeof navigator !== 'undefined' && !navigator.onLine) {
    ...
    switch (code) {
      ...
      case 'not-found':
        return 'The marketingSubscribers collection could not be found.';
      case 'unavailable':
