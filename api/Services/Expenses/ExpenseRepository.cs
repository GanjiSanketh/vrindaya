...
{
    private const string Collection = "expenses";
    private const string CountersCollection = "counters";
    private const string ExpenseCounterDocId = "expenses";

    private readonly IFirebaseService _firebaseService;

    public ExpenseRepository(IFirebaseService firebaseService)
    {
        _firebaseService = firebaseService;
    }

    public async Task<ExpenseDocument?> GetByIdAsync(string id, CancellationToken cancellationToken)
    {
        ...
        return snapshot.Exists ? snapshot.ConvertTo<ExpenseDocument>() : null;
    }

    public async Task<PagedResult<(string Id, ExpenseDocument Data)>> GetAllAsync(
        ...
        DateTime? dateFrom, DateTime? dateTo, CancellationToken cancellationToken)
    {
        ...
        else
        {
            if (!string.IsNullOrWhiteSpace(category))
                baseQuery = baseQuery.WhereEqualTo("expenseCategory", category);
        }

        var clampedPageSize = Math.Clamp(pageSize, 1, 100);
        var orderedQuery = baseQuery.OrderByDescending("expenseDate");
        ...
        var items = snapshot.Documents.Select(d => (d.Id, d.ConvertTo<ExpenseDocument>())).ToList();
        ...
        return new PagedResult<(string Id, ExpenseDocument Data)>
        {
            Items = items,
    ...
    public async Task<string> CreateAsync(ExpenseDocument document, CancellationToken cancellationToken)
    {
        var db = _firebaseService.GetFirestoreDb();
    ...
    public async Task UpdateAsync(string id, ExpenseDocument document, CancellationToken cancellationToken)
    {
        var db = _firebaseService.GetFirestoreDb();
    ...
    public async Task<List<(string Id, ExpenseDocument Data)>> GetAllUnpagedAsync(CancellationToken cancellationToken)
    {
        var db = _firebaseService.GetFirestoreDb();
        var snapshot = await db.Collection(Collection).GetSnapshotAsync(cancellationToken);
        return snapshot.Documents.Select(d => (d.Id, d.ConvertTo<ExpenseDocument>())).ToList();
    }

    public async Task<string> GenerateNextExpenseNumberAsync(CancellationToken cancellationToken)
    {
        var db = _firebaseService.GetFirestoreDb();
        var counterRef = db.Collection(CountersCollection).Document(ExpenseCounterDocId);
