...
export class NewArrivalsPageComponent implements OnInit {
  ...
  private async load(): Promise<void> {
    ...
    this.error.set(null);
    try {
      const page = await this.query.getNewArrivals(PAGE_SIZE);
      this.products.set(page.items);
      this.nextCursor.set(page.nextCursor);
    } catch (err) {
      this.error.set(err instanceof Error ? err.message : 'Could not load new arrivals.');
    } finally {
  ...
  async loadMore(): Promise<void> {
    ...
    try {
      const page = await this.query.getNewArrivals(PAGE_SIZE, cursor);
