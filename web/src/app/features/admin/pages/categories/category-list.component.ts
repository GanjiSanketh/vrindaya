...
import { ApiCategory } from '../../../../core/models/homepage.model';
...
export class CategoryListComponent {
  ...
  async load(): Promise<void> {
    ...
    this.error.set(null);
    try {
      const list = await this.admin.getAllCategories();
  ...
  private async move(from: number, to: number): Promise<void> {
    ...
    try {
      await this.admin.reorderCategories(copy.map(c => c.id));
    } finally {
      this.reordering.set(false);
    }
  }

  async toggleActive(cat: ApiCategory): Promise<void> {
    const updated = await this.admin.updateCategoryStatus(cat.id, !cat.active);
